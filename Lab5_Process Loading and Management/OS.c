...
//		status = OS_AddThread(&SW1_Debounce,128,SW1_Priority);
...
//		status = OS_AddThread(&SW2_Debounce,128,SW2_Priority);
//		if(status == 0){ // thread cannot be created
...
int OS_AddProcess(void(*entry)(void), void *text, void *data, unsigned long stackSize, unsigned long priority){
	...
	newProcess->pid = index+1;	//Current ID is incremented forever for different IDs
	newProcess->code = text;
	newProcess->data = data;
