...
int adcCall(int numArgs, char* args[]) {
	 ...
	 *							 ADC_Collect_old_old(uint32_t channelNum, uint32_t fs, int* data_buffer, uint32_t numberOfSamples)
	...
	*/
	...
	// Command must be: ADC_Collect_old
	else if (numArgs == 5) {
		OutCRLF();
		uint8_t channelNum = parseNumber(args[2]);
		...
		return ADC_Collect_old(channelNum, fs, buf, numSamples);
	}
